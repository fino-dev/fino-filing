...
3 つの Collector はすべて同一の `EdgarClient` を持つ。

- CIK 解決・accession_number 取得など**全 Collector で必要な前段処理**を EdgarClient に集約する
...
| **共有クライアント** | `EdgarClient` が全エンドポイントの HTTP インフラを一元提供。3 つの Collector が共有し、メソッドの呼び出しで用途を切り替える |
