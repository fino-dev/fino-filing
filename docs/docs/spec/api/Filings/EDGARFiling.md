...
Built-in Filing subclass for **one SEC filing** (Submissions filing row + Archives document), e.g. index.htm. Registered on `default_resolver` for Catalog restore. Used by `EdgerArchivesCollector` / `EdgerDocumentsCollector` and `EdgerBulkCollector` (when meta maps to a submission row). Company Facts JSON uses **`EDGARCompanyFactsFiling`** instead.
