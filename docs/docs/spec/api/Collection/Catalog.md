...
WHERE from `expr`, ORDER BY `order_by` (column or `json_extract(data, '$....')`), LIMIT/OFFSET. Returns list of Filing instances.
...
Returns number of rows matching `expr` (or total if `expr` is None).
